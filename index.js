...
client.on('messageCreate', async message => {
    ...
    if (message.mentions.has(client.user.id) && !message.mentions.everyone) {
        ...
        try {
            ...
            // FIXED PAYLOAD: Merged instruction into the contents context map for v1 stability
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    contents: [
                        {
                            role: "user",
                            parts: [{ text: `Instructions to strictly follow for all replies: ${systemInstruction}\n\nUser input: ${userPrompt}` }]
                        }
                    ]
                })
            });
